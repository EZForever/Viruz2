...
public:
  ...
  ~fileM()
  {
	...
	virtual sBool open(sInt resID, LPTSTR typeID=RT_RCDATA, HINSTANCE inst=NULL)
	{
    HRSRC     res;
    HGLOBAL   rptr;
    ...
    if(!inst)
      inst=GetModuleHandle(0);
		if (!(res=FindResource(inst, MAKEINTRESOURCE(resID), typeID)))
		  return sFALSE;
