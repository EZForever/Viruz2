...
static bool SingleChn = false;
static char TempPathName[4096];
static char ModuleFileName[4096]; // path to v2edit library
...
{	
	...
	GetTempFileName(TempPathName,SingleChn?"v2s":"v2e",0,ModuleName);
  ...
  if(module) {
		...
		if(!(v2vstiInit)) {
			...
			if (audioMaster)
			{
				...
				isSynth ();
				programsAreChunks();
				setUniqueID (SingleChn ? 'fV2s' : 'frV2');
			}
		}
	} else
...
//------------------------------------------------------------------------
void ViruzVsti::processReplacing(float **inputs, float **outputs, VstInt32 sampleframes)
{
	v2vstiProcessReplacing(inputs,outputs,sampleframes);
}
//------------------------------------------------------------------------
VstInt32 ViruzVsti::processEvents(VstEvents* events) 
{
...
{
	strcpy (name, SingleChn ? "V2 VSTi (single)" : "V2 VSTi (multi)");
	return true;
}
...
{ 
	strcpy (text, SingleChn ? "farbrausch V2 VSTi (single)" : "farbrausch V2 VSTi (multi)");
	return true; 
}

//-----------------------------------------------------------------------------------------
VstInt32 ViruzVsti::canDo (char* text)
{
...
void* plugin_main(audioMasterCallback audioMaster) 
{	
	// get vst version
...
BOOL WINAPI DllMain (HINSTANCE hInst, DWORD dwReason, LPVOID lpvReserved)
{
	switch(dwReason) {
		case DLL_PROCESS_ATTACH:
			{
	      hInstance = hInst;
	      GetModuleFileName(hInst,ModuleFileName,4096);
	      int c = (int)strlen(ModuleFileName);
	      char* p = ModuleFileName+c-1;	
	      while((*p != '\\')&&(--c))
		      --p;
        SingleChn = !strncmp(p + 1, "v2s", 3);
	      *p = 0;
	      //strcpy(TempPathName,ModuleFileName);
        GetTempPath(4096,TempPathName);
