...
static char g_tempPathName[4096];
static char g_moduleFileName[4096]; // path to v2edit library
static char g_moduleName[4096];
...
public:
    ...
    GetTempFileName(g_tempPathName,SingleChn?"v2s":"v2e",0,g_moduleName);
    CopyFile(g_moduleFileName,g_moduleName,FALSE);

		m_hModule = LoadLibrary(g_moduleName); // load v2edit
...
BOOL WINAPI DllMain (HINSTANCE hInst, DWORD dwReason, LPVOID lpvReserved)
{
	switch(dwReason) {
		case DLL_PROCESS_ATTACH:
			{
 	      g_hInstance = hInst;
	      GetModuleFileName(hInst,g_moduleFileName,4096);
        ...
        GetTempPath(4096, g_tempPathName);
