...
BOOL WINAPI DllMain (HINSTANCE hInst, DWORD dwReason, LPVOID lpvReserved)
{
    ...
    SingleChn = !strncmp(strrchr(moduleFileName, '\\') + 1, "v2s", 3);
...
public:
	...
	virtual bool getRect(ERect **rect)      // CHAOS: cubase fix. this is called once before CreateWindow!
	{
		RECT clientRect;
    if(m_view.m_hWnd!=0)
    {
...
static void Init()
{
	...
	fileM presetbank;
	printf("open: %d\n",presetbank.open(1337,MAKEINTRESOURCE(1337), ATL::_AtlBaseModule.GetResourceInstance()));
	printf("load: %d\n",sdLoad(presetbank));
	presetbank.close();

	//printf("loading bank at %p, size %d\n",presetbank,pbsize);
	//fileM pb;
