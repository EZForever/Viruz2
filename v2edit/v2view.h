...
namespace V2
{
	namespace GUI
	{
		...
		namespace 
		{
			...
			inline CString GetStatusWindowText() { return _T("farbrausch V2 by Tammo Hinrichs and Leonard Ritter"); }
		...
		{   
			...
			static PointF RO3(const PointF& value, const PointF& newRange, const PointF& oldRange)
			{
				return PointF(
			...
			static SizeF RO3(const SizeF& value, const SizeF& newRange, const SizeF& oldRange)
			{
				return SizeF(
			...
			static SizeF GetRectSize(const RectF& rect)
			{
				SizeF size;
				rect.GetSize(&size);
				return size;
			...
			static RECT MakeRECT(const RectF& rectF)
			{
				RECT rect;
			...
			static SizeF MakeSizeF(const PointF& point)
			{
				return SizeF(point.X,point.Y);
			}

			static SizeF Scale(const SizeF& size, float factor)
			{
				return SizeF(size.Width*factor,size.Height*factor);
			}
		

			static RectF DrawGradient(Graphics& graphics, const RectF& rect, const Color& firstColor, const Color& secondColor, REAL angle=90.0f)
			{
			...
			static RectF DrawGradient(Graphics& graphics, const RectF& rect, ColorPreset firstPreset, REAL angle=90.0f)
			{
		...
		public:
			...
			HWND Create(HWND hWndParent=NULL)
			{
				...
				return __super::Create(hWndParent, __super::rcDefault,NULL,windowStyles,windowExStyles);
			}
			...
			PointF GetPosition()
			{
				...
				point.x = rect.left;
				point.y = rect.top;
				if(__super::GetParent())
				{
					::ScreenToClient(__super::GetParent(),&point);
				}
			...
			void Move(const RectF& rect)
			{
        RECT r = MakeRECT(rect);
        __super::MoveWindow(&r);
				m_clientRect = GetClientRect();
			}
			...
			void SetClientSize(const SizeF& size)
			{
				RectF rect(GetRect());
          ...
          window.GetClientRect(&rect);
					rectEnclosing.Union(rectEnclosing,rectEnclosing,RectF((REAL)rect.left, (REAL)rect.top, (REAL)(rect.right - rect.left), (REAL)(rect.bottom - rect.top)));
					window.Detach();
				}
      ...
      static ATL::CWndClassInfoA& GetWndClassInfo() {
        static ATL::CWndClassInfoA wc = { { sizeof(WNDCLASSEX), 0x0002 | 0x0001, __super::StartWindowProc, 0, 0, 0, 0, 0, (HBRUSH)(-1 + 1), 0, "V2Window", 0 }, 0, 0, ((LPSTR)((ULONG_PTR)((WORD)(32512)))), 1, 0, "" }; return wc;
      }
        ...
        __super::UpdateOffscreenBitmap();
			...
			void OnLButtonDown(UINT uMode, const CPoint& position)
			{
        __super::SetFocus();
				SetMsgHandled(FALSE);
			}
		...
		public:
			...
			int Popup(HWND hWnd, const PointF& position)
			{
		...
		public:
			...
			void SetValue(const PointF& value)
			{
			...
			void SetMeasures(const PointF& valueOffset, const SizeF& valueRange)
			{
			...
			{
				SizeF optimalKnobSize = RO3(SizeF(1.0f,1.0f),GetRectSize(__super::m_clientRect),m_valueRange);
			...
			SizeF GetKnobScale()
			{
				return GetRectSize(__super::m_clientRect) - m_knobSize;
			}
			...
			PointF GetKnobOffset()
			{
				...
				if (!m_valueRange.Width)
					knobOffset.X = (__super::m_clientRect.Width/2.0f) - (m_knobSize.Width/2.0f);
				if (!m_valueRange.Height)
					knobOffset.Y = (__super::m_clientRect.Height/2.0f) - (m_knobSize.Height/2.0f);
				return knobOffset;
			}

			void SetKnobSize(const SizeF& knobSize)
			{
				m_knobSize = knobSize;
				SetOptimalKnobSize();
			}
		...
		public:
			...
			{                
				if (m_showKnob)
				{
					switch(vkey)
					{
						...
						case VK_UP:
							{
                __super::SetValue(__super::GetValue() + PointF(1.0f,1.0f));
			...
			{
				x.Format("%i",(int)round(__super::m_value.X));
				y.Format("%i",(int)round(__super::m_value.Y));
			}
			...
			CString GetString()
			{
				...
				GetValueString(x,y);
				CString out;
				switch(__super::GetAxisMode())
				{
          case MeasureControlTemplate<T>::AxisMode::AxisModeX: out.Format("%s",(LPCTSTR)x); break;
					case MeasureControlTemplate<T>::AxisMode::AxisModeY: out.Format("%s",(LPCTSTR)y); break;
					case MeasureControlTemplate<T>::AxisMode::AxisModeXY: out.Format("%s,%s",(LPCTSTR)x,(LPCTSTR)y); break;
				}
				return out;
			}

			void SetCursorOffset(const PointF& offset)
			{
			...
			{       
				if (m_showKnob)
				{
					if (m_anyDrag || __super::PointInKnob(position))
					{
						BeginDrag(MakePointF((LPPOINT)&position));
						SetMsgHandled(FALSE);
					}
					else if (m_quickPick)
					{
            __super::SetValue(__super::ClientToValuePoint(MakePointF((LPPOINT)&position)));
						((T*)this)->OnValueChanged();
						BeginDrag(MakePointF((LPPOINT)&position));
						SetMsgHandled(FALSE);
					}
			...
			void OnLButtonUp(UINT uMode, const CPoint& position)
			{
				if (Dragging())
					EndDrag();
			}

			void OnMouseMove(UINT uMode, const CPoint& position)
			{
				if (Dragging())
					DoDrag(MakePointF((LPPOINT)&position));
			}

			void BeginDrag(const PointF& position)
			{
				m_dragging = TRUE;
				__super::SetCapture();
				PointF knobLocation;
				__super::GetKnobRect().GetLocation(&knobLocation);
				SetCursorOffset(knobLocation - position);
				__super::Invalidate();
			}

			void DoDrag(const PointF& position)
			{
				PointF newKnobLocation = position + m_cursorOffset;
        __super::SetValue(__super::ClientToValuePoint(newKnobLocation));
				((T*)this)->OnValueChanged();
				__super::Invalidate();
			}
			...
			{
				// background
				RectF rectClient = __super::GetClientRect();
			...
			{
				// knob fill
				RectF rectKnob(__super::GetKnobRect());
				DrawGradient(
					graphics,
					rectKnob, __super::Focused()?ColorPresetSliderKnobFillFocused1:ColorPresetSliderKnobFill1,
			...
			void PaintKnobText(Graphics& graphics)
			{
				...
				switch(__super::GetAxisMode())
				{
          case MeasureControlTemplate<T>::AxisMode::AxisModeXY:
					case MeasureControlTemplate<T>::AxisMode::AxisModeX:
						{
							if (rectKnob.X > (__super::m_clientRect.Width/2.0f))
								textLoc.X = rectKnob.X - boundingBox.Width; // put left
							else
								textLoc.X = rectKnob.GetRight()+1; // put right
						} break;
					case MeasureControlTemplate<T>::AxisMode::AxisModeY:
						{
							if (rectKnob.Y > (__super::m_clientRect.Height/2.0f))
		...
		*/
		...
		public:
			...
			void SetDecalBitmapPosition(const PointF& position)
			{
			...
			{
				RectF rectContent = __super::GetClientRect();
			...
			SizeF FindLargestCaption()
			{
				...
				for(typename WindowInfoList::iterator windowInfo = m_windowInfoList.begin(); windowInfo != m_windowInfoList.end(); ++windowInfo)
				{
			...
			void Fit(bool addBorder=true)
			{
				...
				__super::Move(RectF(__super::GetPosition(), SizeF(rect.Width, rect.Height)));
			}
			...
			void Layout(bool calculateLargestCaption=true, bool adjustSizeAfterwards=true, bool breakOnWidth=true)
			{
				...
				if (adjustSizeAfterwards)
				{
					...
					RectF rectGroup = __super::GetRect();
					rectGroup.Width = maxGroupExtends.Width;
					rectGroup.Height = maxGroupExtends.Height;
					__super::Move(rectGroup);
				}
			}

			void SetLargestCaptionSize(const SizeF& largestCaption)
			{
			...
			{
				HWNDList windowList;
				__super::GetChildWindows(windowList);
				((T*)this)->AddWindows(windowList,showCaption,forceBreak);
			}

			void SetWindowCaption(HWND hWnd, LPCTSTR captionText)
			{
				typename WindowInfoList::reverse_iterator windowInfo = FindWindowInfo(hWnd);
			...
			{
				for(typename WindowInfoList::reverse_iterator windowInfo = m_windowInfoList.rbegin(); windowInfo != m_windowInfoList.rend(); ++windowInfo)
				{
					if (windowInfo->hWnd == hWnd)
						return windowInfo;
				}
			...
			{
				// caption
				RectF rectCaption = __super::GetClientRect();
				...
				SWCSPtr ptr(__super::GetWindowText());
          ...
          __super::Invalidate();
			...
			void SetCaptionColor(const Color& firstColor, const Color& secondColor)
			{
        ...
        __super::SetCapture();
			...
			void OnLButtonUp(UINT uMode, const CPoint& position)
			{
				if (Pressed())
				{
					...
					SetPressed(false);
					SetMsgHandled(FALSE);
					if(__super::GetClientRect().Contains(MakePointF((LPPOINT)&position)))
          ...
          __super::Invalidate();
			...
			void Paint(Graphics& graphics)
			{
				...
				PointF location;
				RectF rectString = GetStringRect(__super::GetWindowText(),font,*this);
				location.X = rectCaption.X + rectCaption.Width/2.0f - rectString.Width/2.0f;
				location.Y = rectCaption.Y + rectCaption.Height/2.0f - rectString.Height/2.0f;
				graphics.DrawString(ptr,-1,&font,location,&stringFormat,&textBrush);
		...
		*/
		...
		public:
			...
			HWND Create(HWND hWndParent=NULL, bool readOnly=false)
			{
				...
				HWND hWnd = __super::Create(hWndParent, __super::rcDefault,NULL,windowStyles,0);
        __super::SetFont((HFONT)GetStockObject(DEFAULT_GUI_FONT));
        __super::SetLimitText(255);
				return hWnd;
			}
		...
		public:
			...
			void ValueChanged(bool isDown)
			{
				if (!isDown && __super::m_allowSelect)
				{
            ...
            __super::SetWindowText(m_entryList[m_currentEntry].GetText(m_showIDs));
          ...
          __super::SetFocus();
			...
			void OnSize(UINT uType, CSize size)
			{
				RectF rect(__super::GetClientRect());
			...
			void OnRButtonUp(UINT uMode, const CPoint& position)
			{
				SetMsgHandled(FALSE);
        ...
        __super::Invalidate();
			...
			int Popup()
			{
				RectF rect(__super::GetAbsoluteRect());
		...
		protected:
			...
			public:
				...
				RadioButton(T* buttonGroup, int index)
				{
					...
					m_index = index;
          __super::SetButtonMode(ButtonModeRadio);
				}
			};
			...
			{
				for (typename ButtonList::iterator button = m_buttonList.begin(); button != m_buttonList.end(); ++button)
				{
					if (*(*button))
		...
		protected:
			...
			public:
				...
				void IndexChanged(int index)
				{
          __super::SetParameter(index);
				}

				virtual void Update()
				{
          __super::SetIndex(__super::GetParameter());
				}

				void AddButtons()
				{
					CString strings[16];
					int optionCount = __super::m_info->GetOptions(strings,16);
					for (int index = 0; index < optionCount; index++)
					{
            __super::AddButton(strings[index]);
					}
				}

				virtual void SetFromParameterInfo()
				{
          __super::SetWindowText(__super::m_info->GetName());
					AddButtons();
          __super::Layout();
				}
			...
			public:
				virtual void Update()
				{
					PointF value((REAL)__super::GetParameter(),0.0f);
          __super::SetValue(value);
				}
				...
				{
					PointF value = __super::GetValue();
          __super::SetParameter((int)round(value.X));
				}
          ...
          __super::SetWindowText(__super::m_info->GetName());
					__super::SetMeasures(
						MakePointFromParameterInfoF(__super::m_info),
						MakeSizeFromParameterInfoF(__super::m_info));
				}
			...
			public:
				...
				Window	m_placeHolder[16];
				Window  m_channelCaption;
				Window  m_voiceCaption;
				Window  m_pgmCaption;
				Label	m_pgmLabel[16];
				Label	m_voiceLabel[16+1];
				Label	m_cpuLoadLabel;

        int m_channelCount;
				int m_polyphonyStats[16+1];
				int m_pgmStats[16];
				int m_cpuLoad;
				...
				BOOL OnCreate(LPCREATESTRUCT lpCreateStruct)				
				{
          m_channelCount = SingleChn ? 1 : 16;

					memset(m_polyphonyStats,0,sizeof(int)*(m_channelCount+1));
					memset(m_pgmStats,0,sizeof(int)*m_channelCount);
					m_cpuLoad = 0;
					...
					for (int index = 0; index < (m_channelCount+1); index++)
					{
				...
				void OnDestroy()
				{
					...
					m_voiceLabel[m_channelCount].DestroyWindow();
			...
			*/
			...
			Group* NewGroup(const Color& color1, const Color& color2)
			{
...
//				Move(RectF(GetPosition(),SizeF(900.0f,600.0f)));
		...
		public:
			...
			BEGIN_MSG_MAP_EX(Frame)
				...
				MSG_WM_SIZE(OnSize)
				if (uMsg == 0x0201) {
    this->SetMsgHandled(1); OnLButtonDown((UINT)wParam, ::CPoint(((int)(short)((WORD)(((DWORD_PTR)(lParam)) & 0xffff))), ((int)(short)((WORD)((((DWORD_PTR)(lParam)) >> 16) & 0xffff))))); lResult = 0; if (this->IsMsgHandled()) return 1;
}
