...
void ssTick()
{
	...
	for (sInt ch=0; ch<16; ch++) 
	{
      ...
      if (ch==10)
      {
        char buffer[128]; sprintf(buffer,"pc %d\n",sc.lastpc); OutputDebugStringA(buffer);
      }
...
void __stdcall ssRender(void *, sF32 *outbuf, sU32 len)
{
	...
	if (state.running && !state.silent)
	{
		while (len)
		{
          ...
          sU32 q, r;
          sU8 c;
          q = _udiv64((sU64)(state.nexttime - state.time) * state.usecs, base.timediv2, (unsigned int*)&r);
          c = _addcarry_u32(0, state.smplrem, r, (unsigned int*)&state.smplrem);
          _addcarry_u32(c, state.smpldelta, q, (unsigned int*)&state.smpldelta);
#endif
				}
				else
					state.smpldelta=-1;
...
#else
  bla = (sU64)bla * TIMER_TICKS_PER_SEC / 176400;
#endif
	return bla;
}
...
#else
  destsmpl = (sU64)time * 44100 / TIMER_TICKS_PER_SEC;
#endif
